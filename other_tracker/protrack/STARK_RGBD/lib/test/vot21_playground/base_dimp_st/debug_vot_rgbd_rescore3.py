...
from lib.test.vot21_playground.base_dimp_st.draw_mask import draw_mask
...
class STARK_REF_LT(object):
    ...
    def initialize(self, img_rgb, box, aux_tracker, ar):
        """box: list"""
    ...
    def track(self, img_rgb):
        ...
        mask_pred = self.ar.get_mask(img_rgb, np.array(pred_bbox_ref))
        draw_mask(cv2.cvtColor(img_rgb, cv2.COLOR_BGR2RGB), mask_pred, idx=0, show=True, save_dir=None, th=0.2)

        return pred_bbox_ref, conf_score, _conf_score


class AlpahRefine():
    def __init__(self, image, init_box, model_path='checkpoints/ARcm_r34/SEcmnet_ep0040-a.pth.tar'):
        from pytracking.refine_modules.refine_module import RefineModule
        refine_path = os.path.join(prj_dir, model_path)
        selector_path = 0  # get the mask
        sr = 2.0; input_sz = int(128 * sr)  # 2.0 by default
        self.RF_module = RefineModule(refine_path, selector_path, search_factor=sr, input_sz=input_sz)
        self.initialize(image, init_box)

    def initialize(self, img, init_box):
        cx, cy, w, h = self.get_axis_aligned_bbox(np.array(init_box))
        gt_bbox_ = [cx - (w - 1) / 2, cy - (h - 1) / 2, w, h]
        self.RF_module.initialize(img, np.array(gt_bbox_))

    def get_mask(self, img, init_pred):
        return self.RF_module.get_mask(img, np.array(init_pred))
...
def run_vot_exp(base_tracker, base_param, ref_tracker, ref_param, use_new_box, save_dir, vis=False):
    debug_loader = DBLoader(data_dir='/home/zxy/Desktop/VOT_RGBD/data/RGBD19/box_room_noocc_1')
    torch.set_num_threads(1)
    dir_name = base_tracker + '_' + base_param + '--' + ref_tracker + '_' + ref_param + 'rescore3'
    ...
    ar = AlpahRefine(image, init_box)
    tracker.initialize(image, init_box, dimp, ar)
