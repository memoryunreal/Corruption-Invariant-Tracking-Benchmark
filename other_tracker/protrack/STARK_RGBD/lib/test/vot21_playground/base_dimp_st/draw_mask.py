...
def mask_superpose(frame, mask_pred, color=(0, 255, 0), th=0.1):
    im4show = frame
    mask_pred = np.uint8(mask_pred > th)[:, :, None]
...
def draw_frame_idx(frame, idx):
    ...
    cv2.putText(frame, 'WithMask', (W//15, legend_y + 24),
                cv2.FONT_HERSHEY_SIMPLEX, 1, color=(0, 255, 255), thickness=3)
    return frame


def draw_mask(frame, mask, color=(0, 255, 0), idx=0, show=False, save_dir=None, th=0.1):
    out_image = mask_superpose(frame, mask, color, th=th)
