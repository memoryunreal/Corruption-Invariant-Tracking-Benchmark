...
class DynamicUpdater():
    def __init__(self, score_th=0.3, iou_th=0.75):
        self.identical_cnt = 0
        self.update_semaphore = False
        self.score_th = score_th
        self.iou_th = iou_th
        self.dynamic_cnt_th = 500000

    def eval(self, boxA, boxB, stark_score):
        """ evaluate the quality of predictions """
        iou = self.calc_iou(boxA, boxB)
        if stark_score < self.score_th and iou > self.iou_th:
            self.identical_cnt += 1
        else:
            self.identical_cnt = 0  # 要求连续达标的情况下才更新模板

        if self.identical_cnt > self.dynamic_cnt_th:
            self.identical_cnt = 0
            self.update_semaphore = True
        else:
            self.update_semaphore = False

        bonus = 0.3 * (max(2*(iou-0.5), 0) ** 2)
        return bonus
...
class STARK_REF_LT(object):
    ...
    def initialize(self, img_rgb, box, aux_tracker):
        ...
        self.scope_manager = ScopeManager(self.H, self.W)
    ...
    def track(self, img_rgb):
        ...
        _conf_score = self.me.rescore(np.array(pred_bbox), conf_score)  # depress teleport

        aux_outputs = self.aux_tracker.track(img_rgb)
        aux_pred_bbox = aux_outputs['target_bbox']

        bonus = self.dynamic_updater.eval(pred_bbox, aux_pred_bbox, outputs["conf_score"])
        if self.dynamic_updater.update_semaphore:
            self.tracker.update_template(img_rgb)
            update_flag = True

        if self.scope_manager.out_of_scope or _conf_score < self.dimp_th:  # 如果Stark长期失效，那么super_dimp也不那么可信
            ...
            if self.scope_manager.out_of_scope:
                conf_score = conf_score * 0.1
        else:
        ...
        self.scope_manager.moniter(np.array(outputs['target_bbox']), outputs["conf_score"])

        conf_score = conf_score + bonus
        return pred_bbox_ref, conf_score, _conf_score, self.scope_manager.out_of_scope
...
class ScopeManager():
    def __init__(self, H, W):
        ...
        self.scope_boundary = np.array([3, 3, self.W - 3, self.H - 3])
        self.corner_scope_boundary = self.scope_boundary.copy()  # 如果角点超出此范围，则renwei目标出镜
        self.wh_mem = None
        self.centering_cnt = 0

    def xywh2tlbr(self, box):
        return np.concatenate([box[:2], box[:2] + box[2:]])

    def moniter(self, xywh, score):
        wh = xywh[2:]
        tlbr = self.xywh2tlbr(xywh)
        self.tl = tlbr[:2]
        self.br = tlbr[2:]
        if (self.tl <= self.scope_boundary[:2]).any() or (self.br >= self.scope_boundary[2:]).any():
            self.centering_cnt = 0
            if self.wh_mem is None:
                self.corner_scope_boundary[2:] = self.scope_boundary[2:] - wh/2
                self.corner_scope_boundary[:2] = self.scope_boundary[:2] + wh/2
                self.wh_mem = wh

        else:  # object is not near the boundary
            if score > 0.5:
                self.centering_cnt += 1
            if self.centering_cnt > 5:  # 连续超过5帧远离边界，则认为目标在视野中完整
                self.wh_mem = None
                self.out_of_scope = False

        if self.wh_mem is not None:
            if (self.br <= self.corner_scope_boundary[:2]).any() or (self.tl >= self.corner_scope_boundary[2:]).any():
                """ 如果对侧角点超出范围，则认为目标出镜 """
                self.out_of_scope = True

    def reset(self):
        """ if the object return to the center of scope, reset the scope manager """
        self.out_of_scope = False
        self.scope_boundary = np.array([0, 0, self.W - 1, self.H - 1])
        self.corner_scope_boundary = self.scope_boundary  # 如果角点超出此范围，则renwei目标出镜


def run_vot_exp(base_tracker, base_param, ref_tracker, ref_param, use_new_box, save_dir, vis=False):
    torch.set_num_threads(1)
    dir_name = 'longterm_debug' + base_tracker + '_' + base_param + '--' + ref_tracker + '_' + ref_param + 'rescore3'
    ...
    while True:
        ...
        if not imagefile:
            break
        image = cv2.cvtColor(cv2.imread(imagefile), cv2.COLOR_BGR2RGB)  # Right
        b1, conf, _conf, ofs = tracker.track(image)
        x1, y1, w, h = b1
        ...
        if vis and (cnt % 5) == 0:
            ...
            cv2.putText(image_b, "{}".format(ofs), (100, 400), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 2)
