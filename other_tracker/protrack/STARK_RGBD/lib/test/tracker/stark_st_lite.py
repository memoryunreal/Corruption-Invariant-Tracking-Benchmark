...
import cv2
import os
from lib.utils.merge import merge_template_search_lite
from lib.models.stark import build_starkst_lite
from lib.test.tracker.stark_utils import Preprocessor_lite
from lib.utils.box_ops import clip_box


class STARK_ST_lite(BaseTracker):
    def __init__(self, params, dataset_name):
        super(STARK_ST_lite, self).__init__(params)
        network = build_starkst_lite(params.cfg, pretrained=False)
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        self.cfg = params.cfg
        self.network = network.cuda()
        self.network.eval()
        self.preprocessor = Preprocessor_lite()
        self.state = None
        # for debug
        self.debug = False
        self.frame_id = 0
    ...
    def initialize(self, image, info: dict):
        ...
        z_patch_arr1, _, _ = sample_target(image, info['init_bbox'], self.params.template_factor,
                                           output_sz=self.params.template_size)
        template1 = self.preprocessor.process(z_patch_arr1)
        with torch.no_grad():
    ...
    def track(self, image, info: dict = None, return_update_flag=False):
        ...
        with torch.no_grad():
            ...
            seq_dict = merge_template_search_lite(feat_dict_list)
            # run the transformer
        ...
        if self.save_all_boxes:
            ...
            if return_update_flag:
                return {"target_bbox": self.state,
                        "conf_score": conf_score}, update_flag
            else:
                return {"target_bbox": self.state,
                        "conf_score": conf_score}
