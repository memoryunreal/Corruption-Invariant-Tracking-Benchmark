...
def load_ckpt_from_list(ckpt_list):
    n_ckpt = float(len(ckpt_list))
    print("Using average weights from %d models" % n_ckpt)
    state_dict = {}
    for idx, ckpt_path in enumerate(ckpt_list):
        cur_dict = torch.load(ckpt_path, map_location='cpu')['net']
        if idx == 0:
            state_dict.update(cur_dict)
        else:
            for k, v in state_dict.items():
                state_dict[k] = v + cur_dict[k]
    for k in state_dict.keys():
        state_dict[k] = state_dict[k] / n_ckpt
    return state_dict
...
class STARK_S(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        if isinstance(self.params.checkpoint, list):
            state_dict = load_ckpt_from_list(self.params.checkpoint)
