...
"""2021.4.20 Only replace the original score head with the meta-updater but still updating periodically
such as every 200 frame"""


class STARK_ST_MU_new(BaseTracker):
    def __init__(self, params, dataset_name):
        super(STARK_ST_MU_new, self).__init__(params)
            ...
            for idx, update_i in enumerate(self.update_intervals):
                if self.frame_id % update_i == 0:
                    ...
                    if update:
                        ...
                        with torch.no_grad():
                            z_dict_t = self.network.forward_backbone(template_t)
                            self.z_dict_list[idx+1] = z_dict_t  # the 1st element is template from the 1st frame
