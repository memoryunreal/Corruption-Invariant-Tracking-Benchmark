...
def parameters(yaml_name: str):
    ...
    if cfg.TEST.WEIGHT_AVERAGE:
        params.checkpoint = []
        for i in range(cfg.TEST.AVERAGE_EPOCHS):
            params.checkpoint.append(os.path.join(save_dir, "checkpoints/train/stark_s/%s/STARKS_ep%04d.pth.tar" %
                                                  (yaml_name, cfg.TEST.EPOCH - i)))
    else:
