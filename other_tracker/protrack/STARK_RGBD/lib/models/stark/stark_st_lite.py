...
"""
from .backbone_lite import build_backbone
from .transformer import build_transformer
from .head import build_box_head, MLP
from lib.models.stark.stark_s_lite import STARKS_LITE


class STARKST_LITE(STARKS_LITE):
    ...
    def forward_transformer(self, seq_dict, run_box_head=False, run_cls_head=False):
        ...
        output_embed, enc_mem = self.transformer(seq_dict["feat"], None, self.query_embed.weight,
                                                 None, return_encoder_output=True)
...
def build_starkst_lite(cfg, pretrained=True):
    backbone = build_backbone(cfg, pretrained=pretrained)  # backbone and positional encoding are built together
