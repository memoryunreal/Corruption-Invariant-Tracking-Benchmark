...
"""2021.04.10 Try relative positional encoding"""
from .vision_transformer import Attention


class Transformer_rpe(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerEncoderLayer_rpe(d_model, nhead, dim_feedforward,
        ...
        if num_encoder_layers == 0:
            ...
            self.encoder = TransformerEncoder_rpe(encoder_layer, num_encoder_layers, encoder_norm)
    ...
    def _reset_parameters(self):
        for n, p in self.named_parameters():
            if p.dim() > 1 and "rel_pos_embed" not in n:
                nn.init.xavier_uniform_(p)
...
class TransformerEncoder_rpe(nn.Module):
...
class TransformerEncoderLayer_rpe(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, divide_norm=False):
        super().__init__()
        # self-attention
        self.self_attn = Attention(dim=d_model, num_heads=nhead, attn_drop=dropout, proj_drop=dropout,
                                   relative_position=True, max_relative_position=20)
    ...
    def forward_post(self, src):
        src1 = self.norm1(src + self.dropout1(self.self_attn(src)))
        src2 = self.norm2(src1 + self.dropout2(self.ffn(src1)))
        return src2

    def forward(self, src):
        if self.normalize_before:
            raise ValueError("Pre-Norm is not supported for now.")
...
def build_transformer(cfg):
    if cfg.MODEL.TRANSFORMER.RPE:
        transformer_class = Transformer_rpe
    else:
