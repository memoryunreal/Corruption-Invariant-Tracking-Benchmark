...
from .head import build_box_head, MLP_BN
from lib.models.stark.stark_s import STARKS


class STARKSTBN(STARKS):
    ...
    def forward_head(self, hs, memory, run_box_head=False, run_cls_head=False):
        ...
        if run_cls_head:
            ...
            out_dict.update({'pred_logits': self.cls_head(hs.squeeze())})
        if run_box_head:
...
def build_starkst_bn(cfg):
    """Add BN to the classification head"""
    backbone = build_backbone(cfg)  # backbone and positional encoding are built together
    transformer = build_transformer(cfg)
    box_head = build_box_head(cfg)
    cls_head = MLP_BN(cfg.MODEL.HIDDEN_DIM, cfg.MODEL.HIDDEN_DIM, 1, cfg.MODEL.NLAYER_HEAD)
    model = STARKSTBN(
        backbone,
        transformer,
        box_head,
