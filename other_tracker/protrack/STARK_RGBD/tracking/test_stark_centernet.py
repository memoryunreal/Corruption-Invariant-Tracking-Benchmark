...
if __name__ == "__main__":
    ...
    args = parse_args()
    '''update cfg'''
    yaml_fname = 'experiments/%s/%s.yaml' % (script, config)
    ...
    if script == "stark_s":
    ...
    elif script == "stark_st2" or script == "stark_st_cnt1":
        if script == "stark_st2":
            model_constructor = model_module.build_starkst
        elif script == "stark_st_cnt1":
            model_constructor = model_module.build_starkst_cnt
        else:
            raise ValueError()
        model = model_constructor(cfg)
        # get the template and search
        ...
        oup_dict, _, _ = model(img=None, seq_dict=seq_dict, mode="transformer", run_box_head=True, run_cls_head=True)
        for k, v in oup_dict["pred_boxes"][0].items():
            print(k, v.size())
