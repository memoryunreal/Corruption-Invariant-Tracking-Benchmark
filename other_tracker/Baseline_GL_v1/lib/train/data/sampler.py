...
class TrackingSampler(torch.utils.data.Dataset):
        ...
        while not valid:
            ...
            try:
                ...
                mix_template_frames, mix_template_anno, mix_meta_obj_train = dataset.get_frames(mix_seq_id, mix_template_frame_ids, mix_seq_info_dict)
                        # input shape (h,w,c)
                t_h = template_frames[0].shape[0]
                t_w = template_frames[0].shape[1]
                m_h = mix_template_frames[0].shape[0]
                m_w = mix_template_frames[0].shape[1]

                for id in range(2):
                    if random.uniform(0, 1) >= 0:
                        mixing_coeff=[1.0, 1.0]
                        lam = np.float32(np.random.beta(mixing_coeff[0], mixing_coeff[1]))
                        # lam = 0.7
                        if not t_h == m_h or not t_w == m_w:
                            # resize parameter (w,h)
                            img_sup = cv2.resize(mix_template_frames[id], (t_w, t_h), interpolation = cv2.INTER_AREA)
                        else:
                            img_sup = mix_template_frames[id]
                        ...
                        # cv2.imwrite("/home/CVPR2023/Corruption-Invariant-Tracking-Benchmark/other_tracker/Baseline_GL_v1/debug/temp_mix.png", mix_template_frames[id])
                        template_frames[id] = lam * template_frames[id] + (1 - lam) * img_sup
                        # cv2.imwrite("/home/CVPR2023/Corruption-Invariant-Tracking-Benchmark/other_tracker/Baseline_GL_v1/debug/mix_GL{}.png".format(id), template_frames[id])
    ...
    def get_frame_ids_trident(self, visible):
        ...
        # set earse mix template id temporal mix
        template_frame_ids_mix = []
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0 or None in template_frame_ids_mix:
            template_frame_ids_extra = []
            ...
            for max_gap in self.max_gap:
                ...
                if self.frame_sample_mode == "trident_pro":
                    ...
                    mix_id = self._sample_visible_ids(visible, num_ids=1, min_id=min_id, max_id=max_id,
                                                    allow_invisible=True)
                else:
                    f_id = self._sample_visible_ids(visible, num_ids=1, min_id=min_id, max_id=max_id)
                if f_id is None:
                ...
                if mix_id is None:
                    template_frame_ids_mix += [None]
                else:
                    template_frame_ids_mix += mix_id
                

        template_frame_ids = template_frame_id1 + template_frame_ids_extra + template_frame_ids_mix
