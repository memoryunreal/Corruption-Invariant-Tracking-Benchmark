    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            ...
            if isinstance(data[0], dict):
                ...
                for obj_id, d in data_dict.items():
                    timings_file = '{}_{}_time.value'.format(base_results_path, obj_id)
                    save_time(timings_file, d)
            else:
                timings_file = '{}_001_time.txt'.format(base_results_path)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, blend=0,visual=0, corrupt=0):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, blend=blend)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info, blend=blend, corrupt=corrupt)
...
def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, blend=0, visual=0, corrupt=0):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info, blend=blend, visual=visual, corrupt=corrupt)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, visdom_info, blend, visual, corrupt) for seq, tracker_info in product(dataset, trackers)]
