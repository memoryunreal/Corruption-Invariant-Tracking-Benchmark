...
import logging
log_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], '../log/noise.log')
...
'''
noise_workspace = '/ssd3/lz/TMM2022/workspace/corruption/'
noise_list = ['gaussian','shot','impulse','speckle']
# trackers = ['CSRRGBD++','DSKCF_shape', 'DAL', 'DeT', 'TSDM','DRefine']
trackers = ['DSKCF_shape']

noise_average_fscore = [[] for i in range(len(trackers))]
noise_average_pr = [[] for i in range(len(trackers))]
noise_average_re = [[] for i in range(len(trackers))]
for noise in noise_list:
        all_trackers = [Tracking(tracker, path=os.path.join(noise_workspace, noise, 'results')) for i,tracker in enumerate(trackers)]
        ...
        for id, tracker in enumerate(all_trackers):
            ...
            if tracker.name == trackers[id]:
                noise_average_fscore[id].append(fscore)
                noise_average_pr[id].append(pr)
                noise_average_re[id].append(re)
            print('Trackers: {} noise: {} seq: {} pr: {} re: {} fscore: {}'.format(tracker.name, noise, tracker_seq_number, pr, re, fscore))


for i, sre in enumerate(noise_average_fscore):
    trackerID = trackers[i]
    srefscore = np.mean(np.array(sre))
    srepr = np.mean(np.array(noise_average_pr[i]))
    srere = np.mean(np.array(noise_average_re[i]))
