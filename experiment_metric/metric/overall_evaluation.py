...
##success or pr-re
success_flag = False
seq_list = os.listdir('/ssd3/lz/TMM2022/dataset/alldata')

### dataset
# seq_list = os.listdir("/ssd3/lz/MM2022/dataset/st")
# seq_list = os.listdir("/data1/yjy/rgbd_benchmark/stc_benchmark/RGBDdataset/")
...
# all_trackers = [Tracking(tracker) for tracker in ["keeptrack_origin_default", "keeptrack_prompt_default", "keeptrack0.04","keeptrack0.03"]]
...
# all_trackers = [Tracking(tracker) for tracker in ["CMCT","dimp18_origin","atom_origin","prdimp18_origin","keeptrack_fast_origin"]]
# all_trackers = [Tracking(tracker) for tracker in ['CSR_RGBD++','DSKCF_shape', 'DAL', 'DeT', 'TSDM', 'DAL_normal']]
all_trackers = [Tracking(tracker) for tracker in ['DRefine_normal']]
all_sequence = [Sequence_t(seq) for seq in seq_list]
# plotfile =  open('/ssd3/lz/MM2022/ACM-MM2022/cdtb.txt', 'w')
for i, trackers in enumerate(all_trackers):
    ...
    for sequence in all_sequence:
        ...
        if sequence.name in trackers._seqlist:
            ...
            continue
    if success_flag:
        su = trackers._prre.success
        print('Trackers: {} success: {}'. format(trackers.name, su))
    else:
