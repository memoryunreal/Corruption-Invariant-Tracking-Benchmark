...
# all_trackers = [Tracking(tracker,path='/ssd2/lz/MM2022/depthtrack/results') for tracker in ['transt50', 'transt_prompt']]
trackerlist = ['CMCT', 'DeT']
all_trackers = [Tracking(tracker) for tracker in trackerlist]

# all_trackers = [Tracking(tracker) for tracker in ['LTDSEd']]
all_sequence = [Sequence_t(seq, dataset= '/ssd3/lz/MM2022/dataset/depthtrack/') for seq in seq_list]
plotfile =  open('sequence_overall.txt', 'w')
resultdict = {track: {k:0 for k in seq_list} for track in trackerlist}
for i, trackers in enumerate(all_trackers):
    ...
    for sequence in all_sequence:
        ...
        resultdict[trackers.name][sequence.name] = fscore
...
origin = "DeT"
variance = "CMCT"
perform = {k:0 for k in seq_list}
for index, seq in enumerate(seq_list):
    fscore_origin = resultdict[origin][seq]
    fscore_prompt = resultdict[variance][seq]
    outperform = fscore_prompt - fscore_origin
    perform[seq] = outperform
top_k = dict(sorted(perform.items(), key=lambda e: e[1], reverse=True))
for key, value in top_k.items():

    print('seq: {} origin: {} prompt: {} outperform: {}'.format(key,resultdict[origin][key], resultdict[variance][key], value))
