...
class PrRe(object):
    ...
    def reset(self):
        ...
        self.depthQ = []
        self.attribute_name = []
        self.attribute_value = [[] for i in range(26)]
        self.lt = []
        self.start_frame = 0
        self.max_overlap = 1
    ...
    def success(self):
        ...
        return np.trapz(np.array(succ), x=self.Xaxis)  / self.max_overlap
    @property
    def su_AT(self):
        '''
            Attribute calculate
        ...
        for id in range(len(all_tag)):
            ...
            try:
                succ = [
                    np.sum(i >= thres
                        for i in overlaps).astype(float) / len(overlaps)
                    for thres in self.Xaxis
                ]
            except:
                print('exception')
                succ = [0  for i in self.Xaxis]
            su = np.trapz(np.array(succ), x=self.Xaxis)  / self.max_overlap
            all_su.append(su)
        
        return dict(zip(all_tag,all_su))
    '''
        attribute 
    '''
    @property
