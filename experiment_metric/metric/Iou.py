...
def estimateCenterErr(box_a, box_b):
    if str(box_b[0])=='nan':
        return 0
    
    centerA = np.array([(box_a[0]+ box_a[2]/2),(box_a[1]+box_a[3]/2)]) 
    centerB = np.array([(box_b[0]+ box_b[2]/2),(box_b[1]+box_b[3]/2)]) 
    center_error = np.linalg.norm(centerA-centerB,ord=2)
    return center_error