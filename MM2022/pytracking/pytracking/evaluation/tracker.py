...
log_path = os.path.join('/home/NIPS2022/workspace/vot2022/evaluation_log', 'vot2022.log')
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None, resultname = 'dimp'):
        ...
        if self.run_id is None:
            ...
            self.results_dir = '{}/{}/{}/{}'.format(env.results_path, self.name, self.parameter_name, self.resultname)
            self.segmentation_dir = '{}/{}/{}/{}'.format(env.segmentation_path, self.name, self.parameter_name, self.resultname)
        else:
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None, blend =0, visual=0, corrupt=0):
        ...
        output = self._track_sequence(tracker, seq, init_info, blend=blend, visual=visual, corrupt=corrupt)
        return output

    def _track_sequence(self, tracker, seq, init_info, blend=0, visual=0, corrupt=0):
        # Define outputs
        ...
        '''
            prompt
        '''
        if not blend == 0 and visual==1:
            vis_path = os.path.join("/home/pytracking/visualization/prompt/", seq.name)
            ...
            vis_path = os.path.join("/home/pytracking/visualization/origin/", seq.name)
            score_path = os.path.join(vis_path, 'scoremap')
            impatch_path = os.path.join(vis_path, 'impatch')
            if not os.path.exists(score_path):
                os.makedirs(score_path)
                os.makedirs(impatch_path)
        if not blend ==0:
            dp = self._read_depth_image(seq.depthframes[0])
            image = cv.addWeighted(image, 1-blend, dp, blend, 0.0)
        '''
            prompt
        '''
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            '''
            if not blend==0:
                dp = self._read_depth_image(seq.depthframes[frame_num])
                image = cv.addWeighted(image, 1-blend, dp, blend, 0.0)
            '''
                prompt
            '''
    ...
    def run_vot2020(self, debug=None, visdom_info=None, random_corrupt=0):
        ...
        if random_corrupt:
            corpt_seq = CIT()
            logging.info("tracker: {} tracker.parameter: {} sequence: {} corpt: {} level: {}".format(self.name, self.parameter_name, image_path,corpt_seq.corrupt_func.__name__, corpt_seq.level))
            image = np.asarray(corpt_seq.corrupt_trans(image_path))
    ...
    def _read_depth_image(self, depth_file: str):
        ...
        try:
            ...
            dp = dp
        dp = cv.normalize(dp, None, 0, 255, cv.NORM_MINMAX)
        dp = cv.applyColorMap(np.uint8(dp), 2)
        return cv.cvtColor(dp, cv.COLOR_BGR2RGB)