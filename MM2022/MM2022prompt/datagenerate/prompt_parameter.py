...
log_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], './prompt_paramter.log')
...
class PromptGeneration(Process):
    ...
    def run(self):
        for id, seqpath in enumerate(self.seqpathlist):
            ...
            newcolor_path = os.path.join(seqpath, 'newcolor') 
            ...
            with open(sequencefile, 'w') as f:
                ...
                if suffixname=='jpg\n':
                    linevalue = 'channels.color=color/%08d.png\n'
                    value[0] = linevalue
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="--Dataset path, --num_process")
    parser.add_argument('--dataset', type=str,required=True, help="dataset path")
    parser.add_argument('--workers', type=int, required=True, help="number of process")
    args = parser.parse_args()
    dirpath = args.dataset
    process_num = args.workers
    print('--dataset {} --workers {}'.format(dirpath, process_num))
