...
def rgbd_blend(color,depth,depthth=5000,blend=0.05,style="JET"):
    ...
    if colormap_style == "cv2.COLORMAP_JET":
        style = 2
        ...
        dp = cv2.applyColorMap(np.uint8(dp), style)
    ...
    elif colormap_style == "cv2.COLORMAP_AUTUMN":
        ...
        try:
            ...
            dp = dp
        dp = cv2.normalize(dp, None, 0, 255, cv2.NORM_MINMAX)
        dp = cv2.applyColorMap(np.uint8(dp), cv2.COLORMAP_AUTUMU)
        # dp = cv2.applyColorMap(np.uint8(dp), cv2.COLORMAP_JET)
