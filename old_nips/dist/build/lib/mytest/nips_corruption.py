...
'''
    four noises
'''
def gaussian_noise(x, severity=1, max_value=255.):
    c = [.08, .12, 0.18, 0.26, 0.38][severity - 1]

    x = np.array(x) / max_value
    return np.clip(x + np.random.normal(size=x.shape, scale=c), 0, 1) * max_value

def shot_noise(x, severity=1, max_value=255.):
    c = [60, 25, 12, 5, 3][severity - 1]

    x = np.array(x) / max_value
    return np.clip(np.random.poisson(x * c) / float(c), 0, 1) * max_value


def impulse_noise(x, severity=1, max_value=255.):
    c = [.03, .06, .09, 0.17, 0.27][severity - 1]

    x = sk.util.random_noise(np.array(x) / max_value, mode='s&p', amount=c)
    return np.clip(x, 0, 1) * max_value

def speckle_noise(x, severity=1, max_value=255.):
    c = [.15, .2, 0.35, 0.45, 0.6][severity - 1]

    x = np.array(x) / max_value
    return np.clip(x + x * np.random.normal(size=x.shape, scale=c), 0, 1) * max_value

'''
    Blur
'''
def defocus_blur(x, severity=1, max_value=255.):
    ...
    return np.clip(channels, 0, 1) * max_value

def glass_blur(x, severity=1, max_value=255.):
    # sigma, max_delta, iterations
    c = [(0.7, 1, 2), (0.9, 2, 1), (1, 2, 3), (1.1, 3, 2), (1.5, 4, 2)][
        severity - 1]

    x = np.uint8(
        gaussian(np.array(x) / max_value, sigma=c[0], multichannel=True) * max_value)
    ...
    return np.clip(gaussian(x / max_value, sigma=c[0], multichannel=True), 0,
                   1) * max_value

def motion_blur(x, severity=1,max_value=255.):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        gray = np.clip(np.array(x).transpose((0, 1)), 0, max_value)
...
def zoom_blur(x, severity=1, max_value=255.):
    ...
    x = (np.array(x) / max_value).astype(np.float32)
    out = np.zeros_like(x)
...
'''
    Weather: rain, fog, frost, snow, brightness, spatter
'''
...
def fog(x, severity=1, max_value=255.):
    ...
    if len(x_shape) < 3 or x_shape[2] < 3:
        ...
        x += c[0] * \
             ...
             :shape[1]][..., np.newaxis]
    return np.clip(x * max_val / (max_val + c[0]), 0, 1) * max_value


def frost(x, severity=1, max_value=255.):
    c = [(1, 0.4),
         (0.8, 0.6),
         (0.7, 0.7),
         (0.65, 0.7),
    ...
    return np.clip(c[0] * np.array(x) + c[1] * frost_rescaled, 0, max_value)
...
def snow(x, severity=1, max_value=255.):
    ...
    x = np.array(x, dtype=np.float32) / max_value
    ...
    snow_layer = np.round(snow_layer * max_value).astype(np.uint8) / max_value
    ...
    try:
        return np.clip(x + snow_layer + np.rot90(snow_layer, k=2), 0, 1) * max_value
    except ValueError:
...
def brightness(x, severity=1, max_value=255.):
...
def spatter(x, severity=1, max_value=255.):
    ...
    if c[5] == 0:
        liquid_layer = (liquid_layer * max_value).astype(np.uint8)
        dist = np.uint8(max_value) - cv2.Canny(liquid_layer, 50, 150)
        ...
        color = np.concatenate((175 / max_value * np.ones_like(m[..., :1]),
                                238 / max_value * np.ones_like(m[..., :1]),
                                238 / max_value * np.ones_like(m[..., :1])), axis=2)
        ...
        if len(x.shape) < 3 or x.shape[2] < 3:
            ...
            return np.clip(x + add_spatter_gray, 0, 1) * max_value
        ...
        # mud brown
        color = np.concatenate((63 / max_value * np.ones_like(x_rgb[..., :1]),
                                42 / max_value * np.ones_like(x_rgb[..., :1]),
                                20 / max_value * np.ones_like(x_rgb[..., :1])),
                               axis=2)
        color *= m[..., np.newaxis]
        if len(x.shape) < 3 or x.shape[2] < 3:
            x *= (1 - m)
            return np.clip(x + rgb2gray(color), 0, 1) * max_value

        else:
            x *= (1 - m[..., np.newaxis])
            return np.clip(x + color, 0, 1) * max_value

'''
    Digital: contrast, elastic, pixel, JPEG compression, saturate
'''
def contrast(x, severity=1, max_value=255.):
    c = [0.4, .3, .2, .1, .05][severity - 1]

    x = np.array(x) / max_value
    means = np.mean(x, axis=(0, 1), keepdims=True)
    return np.clip((x - means) * c + means, 0, 1) * max_value

def saturate(x, severity=1, max_value=255.):
...
def elastic_transform(image, severity=1, max_value=255.):
    image = np.array(image, dtype=np.float32) / max_value
    shape = image.shape
    shape_size = shape[:2]

    sigma = np.array(shape_size) * 0.01
    alpha = [max_value * 0.05, max_value * 0.065, max_value * 0.085, max_value * 0.1, max_value * 0.12][
        severity - 1]
